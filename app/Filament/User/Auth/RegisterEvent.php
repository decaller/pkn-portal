...
class RegisterEvent extends BaseRegister
{
    ...
    {
        // First include the existing EventRegistrationForm schema so its fields appear
        // before the registration inputs.
        if (class_exists(EventRegistrationForm::class)) {
            $eventForm = new EventRegistrationForm;

            if (method_exists($eventForm, 'configure')) {
                $schema = $eventForm->configure($schema);
            }
        }
