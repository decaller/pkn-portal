                ...
                Section::make('Event Rundown')
                    ...
                    ->schema([
                        Repeater::make('rundown') // Matches the 'rundown' column in DB
                            ->schema([
                                ...
                                TextInput::make('title')
                                    ...
                                    ->afterStateUpdated(function ($state, $set, $get) {
                                        ...
                                        $eventTitle = $get('../../title') ?? 'event';
                                        
                                        // 2. Combine Event Title + Session Title + Random Suffix
                                        $combinedSlug = Str::slug($eventTitle) . '-' . Str::slug($state) . '-' . Str::lower(Str::random(4));
                                ...
                                FileUpload::make('session_files')
                                    ...
                                    ->directory(fn ($get) => 'events/' . $get('../../slug') . '/sessions')
                                    ->preserveFilenames()
                                    ->downloadable(),
                            ...
                            ->itemLabel(fn (array $state): ?string => $state['title'] ?? null) // Show title on the collapsed bar
