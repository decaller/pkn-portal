<x-dynamic-component :component="$getEntryWrapperView()" :entry="$entry">
    <div class="overflow-hidden rounded-lg border bg-gray-50 dark:bg-gray-900">
        @php
            $state = $getState();
        ...
        @elseif ($ext === 'pdf')
            <iframe src="{{ $url }}" class="block w-full"
                style="display: block; width: 100%; min-width: 100px; min-height: 800px; height: 800px;"
                frameborder="0"></iframe>
        @elseif (in_array($ext, ['doc', 'docx', 'xls', 'xlsx', 'pptx'], true))
            <iframe src="https://view.officeapps.live.com/op/view.aspx?src={{ urlencode($url) }}" class="block w-full"
                style="display: block; width: 100%; min-width: 100px; min-height: 800px; height: 800px;"
                frameborder="0"></iframe>
        @else
